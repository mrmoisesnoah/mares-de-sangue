...
async function telaMesa(id){ layout('<p>Carregando…</p>'); var mesa=S.mesas.find(function(m){return m.id===id;});
  ...
  var secMural=(S.user||muralTot)?'<div class="secao mural-sec">'+secHead(icon("position-marker"),"Mural da Campanha",muralTot,(S.user?'<a class="btn mini" onclick="go(\'nova\',{mesa:\''+id+'\',vis:\'mesa\'})">+ Postar para os jogadores</a>':''))+'<p class="vis-leg" style="margin-top:-6px">Avisos, resumos e materiais que o mestre disponibiliza para os jogadores.</p>'+(muralTot?'<div class="mural">'+muralItensHtml+'</div>':'<div class="empty">Nada no mural ainda.</div>')+'</div>':'';
  var secMapas='<div class="secao">'+secHead(icon("treasure-map"),"Mapas da mesa",mapas.length,(S.user?'<a class="btn mini sec" onclick="go(\'nova\',{mesa:\''+id+'\',tipo:\'mapa\'})">+ Mapa</a>':''))+(mapas.length?'<div class="cards">'+mapas.map(cardMapa).join("")+'</div>':'<div class="empty">Nenhum mapa nesta mesa ainda.</div>')+'</div>';
