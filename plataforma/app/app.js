...
async function abrirNotif(id,lt,lid){ try{ await sb.from("notificacoes").update({lida:true}).eq("id",id); var nn=(S.notifs||[]).find(function(x){return x.id===id;}); if(nn)nn.lida=true; }catch(e){} var n=(S.notifs||[]).find(function(x){return x.id===id;}); if(n&&n.tipo==="pedido"&&lid){ if(lt==="mesa"){ go("editarMesa",lid); return; } if(lt==="personagem"){ go("personagem",lid); return; } if(lt==="mundo"){ if(S.mundo&&S.mundo.id===lid){ go("editarMundo"); return; } toast("Abra o mundo correspondente e use Editar mundo para aprovar o pedido.","erro"); render(); return; } } var rt={publicacao:"pub",personagem:"personagem",jornal:"jornal",mesa:"mesa"}[lt]; if(rt&&lid){ go(rt,lid); } else { render(); } }
