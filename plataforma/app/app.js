...
async function telaHome(){
  ...
  var _mr=S.user?await meusRascunhos():[]; var _lr=S.user?rascunhosLocais():[]; var rasHtml=(_mr.length+_lr.length)?'<h2>'+icon("quill-ink")+' Seus rascunhos</h2><p class="vis-leg" style="margin-top:-6px">Continue de onde parou. <a onclick="go(\'rascunhos\')">ver todos</a></p><div class="cards">'+_mr.slice(0,3).map(function(p){return '<div class="card clic" onclick="go(\'editar\',\''+p.id+'\')"><div class="dash-ic">'+icon("quill-ink")+'</div><h3>'+esc(p.titulo||"(sem título)")+'</h3><p class="res">rascunho · '+esc(p.tipo||"")+'</p></div>';}).join("")+_lr.slice(0,3).map(function(d){return '<div class="card clic" onclick="resumirRascunhoLocal(\''+d.hash+'\')"><div class="dash-ic">'+icon("quill-ink")+'</div><h3>'+esc(trechoHtml(d.html)||"(rascunho)")+'</h3><p class="res">não salvo neste navegador</p></div>';}).join("")+'</div>':'';
  layout(topo+favHtml+rasHtml+mundosHtml+contHtml+novHtml);
}
...
function initQuills(){ var c=document.getElementById("f_corpo_ql"); if(!c||!window.Quill){ S.quills=null; return; } if(!window.__qlStyles){ window.__qlStyles=true; ["size","align","direction","color","background","font"].forEach(function(n){ try{ var A=Quill.import("attributors/style/"+n); if(A) Quill.register(A,true); }catch(e){} }); } var q=new Quill(c,{ theme:"snow", placeholder:"Escreva o conteúdo…", modules:{ toolbar:{ container:[[{header:[2,3,false]}],["bold","italic","underline","strike"],[{color:[]},{background:[]}],["blockquote"],[{list:"ordered"},{list:"bullet"}],[{align:[]}],["link","image"],["clean"]], handlers:{ image:function(){ quillEscolherImg(q); } } } } }); if(S.quillInit){ q.clipboard.dangerouslyPasteHTML(S.quillInit); } q.on("text-change",function(delta,old,source){ if(source!=="user") return; window.__editDirty=true; clearTimeout(window.__draftT); window.__draftT=setTimeout(salvarRascunho,800); }); if(!window.__beforeUnload){ window.__beforeUnload=true; window.addEventListener("beforeunload",function(e){ if(window.__editDirty){ e.preventDefault(); e.returnValue=""; return ""; } }); } try{ var _raw=localStorage.getItem(draftKey()); if(_raw){ var _d=JSON.parse(_raw); if(_d&&_d.html&&_d.html!==(S.quillInit||"")){ if(window.__autoRestore){ window.__autoRestore=null; q.clipboard.dangerouslyPasteHTML(_d.html); } else { mostrarBarraRascunho(q,_d); } } } }catch(_er){} q.root.addEventListener("paste",function(e){ var cd=e.clipboardData||window.clipboardData; if(!cd) return; var it=cd.items||[]; for(var i=0;i<it.length;i++){ if(it[i].type&&it[i].type.indexOf("image")===0){ var f=it[i].getAsFile(); if(f){ e.preventDefault(); quillSubirImg(q,f); return; } } } var htmlP=""; try{ htmlP=cd.getData("text/html"); }catch(_e){} var txt=""; try{ txt=cd.getData("text/plain"); }catch(_e2){} if((!htmlP||!htmlP.trim()) && txt && pareceMarkdown(txt)){ e.preventDefault(); var sel=q.getSelection(true)||{index:q.getLength()}; q.clipboard.dangerouslyPasteHTML(sel.index, mdToHtmlRaw(txt), "user"); } }); if(!window.__focoEsc){ window.__focoEsc=true; document.addEventListener("keydown",function(e){ if(e.key==="Escape"){ var w=document.getElementById("f_corpo_wrap"); if(w&&w.classList.contains("foco")) toggleFoco(); } }); } initImgTools(q); initImgDrag(q); S.quills={ f_corpo:q }; }
...
function draftKey(){ return "mds:draft:"+(S.user?S.user.id:"x")+":"+location.hash; }
...
async function meusRascunhos(){ if(!S.user) return []; var r=await sb.from("publicacoes").select("*").eq("autor_id",S.user.id).eq("estado","rascunho").order("criado_em",{ascending:false}); return r.error?[]:(r.data||[]); }
function rascunhosLocais(){ var out=[]; try{ var pre="mds:draft:"+(S.user?S.user.id:"x")+":"; for(var i=0;i<localStorage.length;i++){ var k=localStorage.key(i); if(k&&k.indexOf(pre)===0){ try{ var d=JSON.parse(localStorage.getItem(k)); if(d&&d.html){ out.push({hash:k.slice(pre.length),html:d.html,ts:d.ts||0,key:k}); } }catch(e){} } } }catch(e){} out.sort(function(a,b){return b.ts-a.ts;}); return out; }
function trechoHtml(h){ var t=(""+(h||"")).replace(/<[^>]+>/g," ").replace(/\s+/g," ").trim(); return t.length>90?t.slice(0,90)+"…":t; }
function resumirRascunhoLocal(hash){ if(!hash) return; window.__autoRestore=hash; if(location.hash===hash){ rota(); } else { location.hash=hash; } }
...
async function telaRascunhos(){ if(!S.user){ go("login"); return; } layout('<p>Carregando…</p>'); var dbR=await meusRascunhos(); var locR=rascunhosLocais();
  ...
  var locHtml=locR.length?'<ul class="lista2">'+locR.map(function(d){ var quando=""; try{quando=new Date(d.ts).toLocaleString("pt-BR");}catch(e){} return '<li class="li-clic" onclick="resumirRascunhoLocal(\''+d.hash+'\')"><span class="li-tit">'+icon("quill-ink")+' '+esc(trechoHtml(d.html)||"(rascunho vazio)")+'</span><span class="chip">não salvo'+(quando?" · "+esc(quando):"")+'</span> <button class="btn mini sec" style="border-color:#b23b3b" onclick="event.stopPropagation();descartarRascunhoLocal(\''+d.key+'\')">descartar</button></li>'; }).join("")+'</ul>':'';
