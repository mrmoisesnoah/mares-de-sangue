...
function imgPop(){ if(_imgPop) return _imgPop; var d=document.createElement("div"); d.className="img-pop"; d.innerHTML='<div class="img-pop-row"><span class="img-pop-lab">Tamanho</span><button type="button" data-tam="30%">P</button><button type="button" data-tam="50%">M</button><button type="button" data-tam="75%">G</button><button type="button" data-tam="100%">Total</button></div><div class="img-pop-row"><span class="img-pop-lab">Contorno</span><button type="button" data-flo="left" title="Imagem a esquerda, texto a direita">&#8676;</button><button type="button" data-flo="center" title="Centralizada">&#8597;</button><button type="button" data-flo="right" title="Imagem a direita, texto a esquerda">&#8677;</button><button type="button" data-flo="none" title="Sem contorno (alinhar)">&#10005;</button></div><div class="img-pop-row"><span class="img-pop-lab">Alinhar</span><button type="button" data-alin="" title="Esquerda">&#8592;</button><button type="button" data-alin="center" title="Centro">&#9744;</button><button type="button" data-alin="right" title="Direita">&#8594;</button></div>'; d.addEventListener("mousedown",function(e){ e.preventDefault(); }); d.addEventListener("click",function(e){ var b=e.target.closest?e.target.closest("button"):null; if(!b) return; if(b.hasAttribute("data-tam")) imgTam(b.getAttribute("data-tam")); else if(b.hasAttribute("data-flo")) imgContorno(b.getAttribute("data-flo")); else if(b.hasAttribute("data-alin")) imgAlin(b.getAttribute("data-alin")); }); document.body.appendChild(d); _imgPop=d; return d; }
...
function initImgMove(q){ var root=q.root; function marcar(){ try{ var ims=root.querySelectorAll("img"); for(var i=0;i<ims.length;i++) ims[i].setAttribute("draggable","false"); }catch(_a){} } marcar(); q.on("text-change",marcar); setTimeout(marcar,300); root.addEventListener("dragstart",function(e){ if(e.target&&e.target.tagName==="IMG") e.preventDefault(); }); root.addEventListener("mousedown",function(e){ var img=e.target; if(!img||img.tagName!=="IMG"||e.button!==0) return; var sx=e.clientX,sy=e.clientY,drag=false,ghost=null,cur=img,src=img.getAttribute("src"),width=img.getAttribute("width"),style=img.getAttribute("style"); function aplicar(nn){ if(nn&&nn.tagName==="IMG"){ if(width)nn.setAttribute("width",width); if(style)nn.setAttribute("style",style); nn.setAttribute("draggable","false"); nn.classList.add("img-arrastando"); cur=nn; } } function mv(ev){ if(!drag){ if(Math.abs(ev.clientX-sx)+Math.abs(ev.clientY-sy)<6) return; drag=true; esconderImgPop(); if(cur) cur.classList.add("img-arrastando"); ghost=document.createElement("img"); ghost.src=src; ghost.className="img-ghost"; document.body.appendChild(ghost); } ev.preventDefault(); if(ghost){ ghost.style.left=(ev.clientX+12)+"px"; ghost.style.top=(ev.clientY+12)+"px"; } if(document.elementFromPoint(ev.clientX,ev.clientY)===cur) return; var target=caretIdxDePonto(q,ev.clientX,ev.clientY); if(target==null) return; var c=-1; try{ c=q.getIndex(Quill.find(cur)); }catch(_c){ return; } if(c<0) return; var insAt=target>c?target-1:target; if(insAt<0) insAt=0; if(insAt===c) return; q.deleteText(c,1,"silent"); q.insertEmbed(insAt,"image",src,"silent"); try{ var lf=q.getLeaf(insAt+1); var nn=lf&&lf[0]&&lf[0].domNode; if(!nn||nn.tagName!=="IMG"){ var lf2=q.getLeaf(insAt); nn=lf2&&lf2[0]&&lf2[0].domNode; } aplicar(nn); }catch(_n){} } function up(ev){ document.removeEventListener("mousemove",mv,true); document.removeEventListener("mouseup",up,true); if(ghost) ghost.remove(); if(!drag) return; ev.preventDefault(); if(cur) cur.classList.remove("img-arrastando"); var fin=-1; try{ fin=q.getIndex(Quill.find(cur)); }catch(_f){} if(fin>=0) q.setSelection(fin+1,0,"user"); window.__editDirty=true; clearTimeout(window.__draftT); window.__draftT=setTimeout(salvarRascunho,300); } document.addEventListener("mousemove",mv,true); document.addEventListener("mouseup",up,true); }); }
