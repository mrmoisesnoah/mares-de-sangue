...
async function telaSessao(id){ layout('<p>Carregando…</p>'); var se=await umaSessao(id); if(!se){ layout('<div class="aviso">Sessão não encontrada.</div>'); return; }
  ...
  var secRec='<h2>'+icon("round-star")+' Recompensas (XP, itens, prêmios)</h2>'+(ehMestre?'<p><a class="btn sec" onclick="go(\'novaRecompensa\',\''+id+'\')">+ Adicionar recompensa</a></p>':'')+(recs.length?renderRecsHTML(recs,persMesa,ehMestre,id,recPinSet):'<div class="empty">Nenhuma recompensa registrada nesta sessão ainda.</div>');
...
function renderRecsHTML(recs, persMesa, ehMestre, sid, pinSet){ pinSet=pinSet||{}; var nm=function(pid){ if(!pid)return "Todos"; var x=(persMesa||[]).find(function(p){return p.id===pid;}); return x?x.nome:"(personagem)"; };
  var ic=function(t){ return t==="xp"?"⭐":(t==="item"?"⚔":"🏆"); };
  return '<ul class="lista2">'+recs.map(function(r){ var fix=!!pinSet[r.id]; var tit=esc(r.resumo||r.titulo); var sub=(r.resumo&&r.titulo&&r.resumo!==r.titulo)?'<span class="rec-sub">'+esc(r.titulo)+'</span>':''; var acts=ehMestre?'<span class="rec-acts" onclick="event.stopPropagation()"><button type="button" class="rec-pin'+(fix?' on':'')+'" title="'+(fix?'Remover do mural':'Fixar no mural da mesa')+'" onclick="fixarRecompensaNoMural(\''+r.id+'\',\''+r.mesa_id+'\','+(fix?'true':'false')+',this)">'+icon("position-marker")+'</button> <a title="Editar" onclick="go(\'editarRecompensa\',\''+r.id+'\')">✎</a> <a title="Excluir" onclick="excluirRecompensa(\''+r.id+'\',\''+sid+'\')">🗑</a></span>':''; return '<li class="li-clic rec-li" onclick="go(\'recompensa\',\''+r.id+'\')"><span class="li-ic">'+ic(r.tipo)+'</span><span class="li-tit">'+tit+sub+'</span><span class="tipo">'+esc(nm(r.personagem_id))+'</span>'+acts+'</li>'; }).join("")+'</ul>'; }
...
async function telaRecompensa(id){ layout('<p>Carregando…</p>'); var r=await umaRecompensa(id); if(!r){ layout('<div class="aviso">Recompensa não encontrada ou sem permissão.</div>'); return; }
  ...
  layout('<div class="bread"><a onclick="go(\'sessao\',\''+r.sessao_id+'\')">‹ Sessão</a> › Recompensa</div>'
    +'<div class="rec-det"><div class="rec-det-head"><span class="rec-det-ic">'+ic+'</span><div><h1>'+esc(r.resumo||r.titulo)+'</h1><p class="vis-leg">'+esc(tipoLabel)+' · para '+esc(nm)+legExtra+'</p></div></div>'+acts+corpo+'</div>'); }
...
function muralItem(p, mesaId, podeDespinar, kind){ kind=kind||"publicacao"; var rec=(kind==="recompensa"); var alvo=rec?("go('recompensa','"+p.id+"')"):("go('pub','"+p.id+"')"); var tipoLabel=rec?"Recompensa":esc(p.tipo); var titulo=rec?esc(p.resumo||p.titulo):esc(p.titulo); return '<a class="mural-item" onclick="'+alvo+'"><span class="mural-pin">'+icon("position-marker")+'</span>'+(podeDespinar?'<button type="button" class="mural-x" title="Remover do mural" onclick="event.stopPropagation();despinarMural(\''+mesaId+'\',\''+p.id+'\',\''+kind+'\')">✕</button>':'')+'<span class="mural-tipo">'+tipoLabel+'</span><span class="mural-tit">'+titulo+'</span></a>'; }
