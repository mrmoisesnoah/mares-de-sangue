...
function renderRecsHTML(recs, persMesa, ehMestre, sid){ var nm=function(pid){ if(!pid)return "Todos"; var x=(persMesa||[]).find(function(p){return p.id===pid;}); return x?x.nome:"(personagem)"; };
  ...
  return '<ul class="lista2">'+recs.map(function(r){ return '<li class="li-clic" style="cursor:default"><span class="li-ic">'+ic(r.tipo)+'</span><span class="li-tit">'+esc(r.titulo)+(r.tipo==="xp"&&r.quantidade!=null?' ('+r.quantidade+' XP)':'')+'</span><span class="tipo">'+esc(nm(r.personagem_id))+'</span>'+(r.descricao?'<div class="rec-obs">'+r.descricao+'</div>':'')+(ehMestre?' <a onclick="go(\'editarRecompensa\',\''+r.id+'\')">✎</a> <a onclick="excluirRecompensa(\''+r.id+'\',\''+sid+'\')">🗑</a>':'')+'</li>'; }).join("")+'</ul>'; }
...
async function salvarRecompensa(sid, mid, editId){ try{ var t=val("rc_titulo").trim(); if(!t)return erro("Dê um título à recompensa."); var q=val("rc_qtd").trim();
  var d=corpoDoEditor(); var reg={ tipo:val("rc_tipo"), titulo:t, personagem_id:(val("rc_pers")||null), quantidade:(q!==""?parseInt(q,10):null), descricao:(d.corpo||null) };
