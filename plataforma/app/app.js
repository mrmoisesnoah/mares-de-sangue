...
function cardMapa(p){ var img=p.capa_url; var face=img?'<div class="ms-body" style="background-image:url('+esc(img)+')"></div>':'<div class="ms-body ms-noimg"></div>'; var sub=(p.tags&&p.tags.length)?'<p class="mp-sub">'+esc(p.tags[0])+'</p>':''; return '<div class="mapa-scroll clic" onclick="go(\'pub\',\''+p.id+'\')" title="'+esc(p.titulo)+'"><div class="ms-roll">'+face+'<span class="ms-rose">'+ROSA_SVG+'</span></div><div class="mp-kick">MAPA</div><h3 class="mp-t">'+esc(p.titulo)+'</h3>'+sub+'</div>'; }
