...
27. `migracao-chat-gerenciar.sql` — gerenciamento de conversas (per-user em `conversa_membros`): colunas `arquivada`/`fixada`/`oculta_em`; `listar_conversas(p_arquivadas)` e `nao_lidas_total()` respeitam arquivada/oculta_em; RPCs `arquivar_conversa`, `fixar_conversa`, `excluir_conversa` (limpa da minha lista + esconde histórico até agora; reaparece se chegar msg nova), `marcar_nao_lida`. Rodar DEPOIS de `migracao-chat.sql`. Termina com `notify pgrst,'reload schema';`.
